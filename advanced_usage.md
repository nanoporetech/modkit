...
Options:
  ...
  -i, --interval-size <INTERVAL_SIZE>
      ...
      --num-reads <NUM_READS>
          ...
          evenly across aligned regions. If a region is specified, either with the --region option
          or the --sample-region option, then reads will be sampled evenly across the interval
          given. This option is useful for large BAM files. In practice, 50-100 thousand reads is
          sufficient to estimate the model output distribution and determine the filtering threshold
...
Options:
  -f, --sampling-frac <SAMPLING_FRAC>  Fraction of reads to sample, for example 0.1 will sample
                                       1/10th of the reads [default: 0.1]
      --num-reads <NUM_READS>          Max number of reads to use, especially recommended when using
                                       a large BAM without an index
...
Options:
  -t, --threads <THREADS>
      ...
      --filter-percentile <FILTER_PERCENTILE>
          Filter out mod-calls where the probability of the predicted variant is below this
          percentile. For example, 0.1 will filter out the 10% lowest confidence modification calls
